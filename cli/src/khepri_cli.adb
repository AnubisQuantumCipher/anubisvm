...
package body Khepri_CLI is
   ...
   begin
      ...
      Create_Directory (Project_Dir & "/obj");
      Create_Directory (Project_Dir & "/bin");

      --  Create .gpr file for building
      Print_Progress (2, 5, "Creating GPR project file...");
      declare
         GPR_File : File_Type;
      begin
         Create (GPR_File, Out_File, Project_Dir & "/" & Project_Name & ".gpr");
         Put_Line (GPR_File, "-- " & Project_Name & " - KHEPRI Smart Contract Project");
         Put_Line (GPR_File, "-- Generated by khepri init");
         Put_Line (GPR_File, "");
         Put_Line (GPR_File, "project " & Project_Name & " is");
         Put_Line (GPR_File, "");
         Put_Line (GPR_File, "   for Source_Dirs use (""src"");");
         Put_Line (GPR_File, "   for Object_Dir use ""obj"";");
         Put_Line (GPR_File, "   for Exec_Dir use ""bin"";");
         Put_Line (GPR_File, "   for Main use (""" & Project_Name & "_main.adb"");");
         Put_Line (GPR_File, "   for Create_Missing_Dirs use ""True"";");
         Put_Line (GPR_File, "");
         Put_Line (GPR_File, "   package Compiler is");
         Put_Line (GPR_File, "      for Default_Switches (""Ada"") use");
         Put_Line (GPR_File, "        (""-gnatwa"",       -- All warnings");
         Put_Line (GPR_File, "         ""-gnat2012"",     -- Ada 2012");
         Put_Line (GPR_File, "         ""-gnata"",        -- Enable assertions");
         Put_Line (GPR_File, "         ""-g"",            -- Debug info");
         Put_Line (GPR_File, "         ""-O2"");          -- Optimization");
         Put_Line (GPR_File, "   end Compiler;");
         Put_Line (GPR_File, "");
         Put_Line (GPR_File, "   package Binder is");
         Put_Line (GPR_File, "      for Default_Switches (""Ada"") use (""-Es"");");
         Put_Line (GPR_File, "   end Binder;");
         Put_Line (GPR_File, "");
         ...
         Put_Line (GPR_File, "");
         Put_Line (GPR_File, "end " & Project_Name & ";");
         Close (GPR_File);
      end;

      --  Create khepri.toml
      Print_Progress (3, 5, "Creating project configuration...");
      declare
      ...
      Print_Progress (4, 5, "Creating contract template...");
      declare
         Spec_File : File_Type;
         Body_File : File_Type;
         Main_File : File_Type;
      begin
         ...
         Put_Line (Spec_File, "      Counter     : Unsigned_64;");
         Put_Line (Spec_File, "   end record;");
         Put_Line (Spec_File, "");
         Put_Line (Spec_File, "   Empty_State : constant Contract_State := (");
         Put_Line (Spec_File, "      Initialized => False,");
         Put_Line (Spec_File, "      Counter     => 0);");
         Put_Line (Spec_File, "");
         Put_Line (Spec_File, "   --  Initialize the contract");
         Put_Line (Spec_File, "   procedure Initialize (State : in out Contract_State)");
         Put_Line (Spec_File, "      with Global => null,");
         Put_Line (Spec_File, "           Pre    => not State.Initialized,");
         Put_Line (Spec_File, "           Post   => State.Initialized;");
         Put_Line (Spec_File, "");
         Put_Line (Spec_File, "   --  Increment counter");
         Put_Line (Spec_File, "   procedure Increment (State : in out Contract_State)");
         Put_Line (Spec_File, "      with Global => null,");
         Put_Line (Spec_File, "           Pre    => State.Initialized and State.Counter < Unsigned_64'Last,");
         Put_Line (Spec_File, "           Post   => State.Counter = State.Counter'Old + 1;");
         Put_Line (Spec_File, "");
         Put_Line (Spec_File, "   --  Get counter value");
         Put_Line (Spec_File, "   function Get_Counter (State : Contract_State) return Unsigned_64");
         Put_Line (Spec_File, "      with Global => null;");
         Put_Line (Spec_File, "");
         ...
         Put_Line (Body_File, "");
         Put_Line (Body_File, "   procedure Initialize (State : in Out Contract_State) is");
         Put_Line (Body_File, "   begin");
         Put_Line (Body_File, "      State.Initialized := True;");
         Put_Line (Body_File, "      State.Counter := 0;");
         Put_Line (Body_File, "   end Initialize;");
         Put_Line (Body_File, "");
         Put_Line (Body_File, "   procedure Increment (State : in Out Contract_State) is");
         Put_Line (Body_File, "   begin");
         Put_Line (Body_File, "      State.Counter := State.Counter + 1;");
         Put_Line (Body_File, "   end Increment;");
         Put_Line (Body_File, "");
         Put_Line (Body_File, "   function Get_Counter (State : Contract_State) return Unsigned_64 is");
         Put_Line (Body_File, "   begin");
         Put_Line (Body_File, "      return State.Counter;");
         Put_Line (Body_File, "   end Get_Counter;");
         Put_Line (Body_File, "");
         Put_Line (Body_File, "end " & Project_Name & ";");
         Close (Body_File);

         --  Main procedure for testing
         Create (Main_File, Out_File, Project_Dir & "/src/" & Project_Name & "_main.adb");
         Put_Line (Main_File, "--  " & Project_Name & " Test Runner");
         Put_Line (Main_File, "with Ada.Text_IO; use Ada.Text_IO;");
         Put_Line (Main_File, "with Interfaces;");
         Put_Line (Main_File, "with " & Project_Name & "; use " & Project_Name & ";");
         Put_Line (Main_File, "");
         Put_Line (Main_File, "procedure " & Project_Name & "_Main is");
         Put_Line (Main_File, "   State : Contract_State := Empty_State;");
         Put_Line (Main_File, "begin");
         Put_Line (Main_File, "   Put_Line (""KHEPRI Contract Test: " & Project_Name & """);");
         Put_Line (Main_File, "   Put_Line (""==========================="");");
         Put_Line (Main_File, "   New_Line;");
         Put_Line (Main_File, "");
         Put_Line (Main_File, "   Put_Line (""Initializing contract..."");");
         Put_Line (Main_File, "   Initialize (State);");
         Put_Line (Main_File, "   Put_Line (""  Initialized: "" & Boolean'Image (State.Initialized));");
         Put_Line (Main_File, "");
         Put_Line (Main_File, "   Put_Line (""Incrementing counter 3 times..."");");
         Put_Line (Main_File, "   Increment (State);");
         Put_Line (Main_File, "   Increment (State);");
         Put_Line (Main_File, "   Increment (State);");
         Put_Line (Main_File, "   Put_Line (""  Counter: "" & Interfaces.Unsigned_64'Image (Get_Counter (State)));");
         Put_Line (Main_File, "");
         Put_Line (Main_File, "   New_Line;");
         Put_Line (Main_File, "   Put_Line (""All tests passed!"");");
         Put_Line (Main_File, "end " & Project_Name & "_Main;");
         Close (Main_File);
      end;

      --  Create test file
      Print_Progress (5, 5, "Creating test template...");
      declare
         Test_File : File_Type;
      begin
      ...
      Put_Line ("  gprbuild -P " & Project_Name & ".gpr");
      Put_Line ("  ./bin/" & Project_Name & "_main");
      New_Line;
      Put_Line ("Or use the CLI:");
      Put_Line ("  khepri build " & Project_Name);
      Put_Line ("  khepri run " & Project_Name);
   end Handle_Init;
   ...
   procedure Handle_Deploy (Options : Deploy_Options) is
      ...
      --  Helper to convert bounded string to String
      function Get_Contract_Name return String is
      begin
         return Name_Strings.To_String (Options.Contract_Name);
      end Get_Contract_Name;

      function Get_Key_Name return String is
      begin
         return Name_Strings.To_String (Options.Key_Name);
      end Get_Key_Name;
      ...
      --  Contract binary
      Contract_Name_Str : constant String := Get_Contract_Name;
      Binary_Path : constant String := "bin/" & Contract_Name_Str & "_main";
      Binary_File : Ada.Streams.Stream_IO.File_Type;
      Binary_Size : Natural;
      Binary_Data : Aegis_VM_Types.Byte_Array (0 .. 1024 * 1024 - 1) := (others => 0);

      --  Deployment transaction
      Tx_Data    : Aegis_VM_Types.Byte_Array (0 .. 1024 * 1024 + 256 - 1) := (others => 0);
      Tx_Size    : Natural;
      ...
      --  Result
      Tx_Hash    : Aegis_VM_Types.Hash256;
      Receipt    : Transaction_Receipt;
      Error      : RPC_Error;
      Success    : Boolean;
      Found      : Boolean;
   begin
      Print_Info ("Deploying contract: " & Contract_Name_Str);
      Print_Info ("Network: " & Network_To_String (Options.Network));
      New_Line;

      --  Set endpoint based on network
      case Options.Network is
         when Network_Mainnet =>
            Endpoint := Parse_Endpoint ("http://localhost:26657");
         when Network_Testnet =>
            Endpoint := Parse_Endpoint ("http://localhost:26657");
         when Network_Local =>
            Endpoint := Parse_Endpoint ("http://localhost:26657");
         when Network_Custom =>
      ...
      if not Ada.Directories.Exists (Binary_Path) then
         Print_Error ("Contract binary not found: " & Binary_Path);
         Print_Info ("Run 'khepri build' first to compile the contract");
         Set_Exit_Status (Ada.Command_Line.Failure);
         return;
      end if;

      --  Load binary
      Print_Progress (1, 5, "Loading contract binary...");
      Open (Binary_File, In_File, Binary_Path);
      Binary_Size := Natural (Ada.Directories.Size (Binary_Path));

      if Binary_Size > Binary_Data'Length then
         Print_Error ("Contract binary too large: " & Binary_Size'Image & " bytes");
         Close (Binary_File);
         Set_Exit_Status (Ada.Command_Line.Failure);
         return;
      end if;
      ...
      declare
         Elem : Ada.Streams.Stream_Element;
      begin
         for I in 0 .. Binary_Size - 1 loop
            Ada.Streams.Stream_Element'Read (Stream (Binary_File), Elem);
            Binary_Data (I) := Aegis_VM_Types.Byte (Elem);
         end loop;
      end;
      Close (Binary_File);
      Print_Info ("  Binary size: " & Binary_Size'Image & " bytes");

      --  Load signing key from keys directory
      Print_Progress (2, 5, "Loading signing key...");
      declare
         Key_Name_Str : constant String := Get_Key_Name;
         Key_Dir      : constant String := Ada.Environment_Variables.Value ("HOME") &
                                          "/.khepri/keys/";
         SK_Path      : constant String := Key_Dir & Key_Name_Str & ".sk";
         PK_Path      : constant String := Key_Dir & Key_Name_Str & ".pk";
         SK_File      : Ada.Streams.Stream_IO.File_Type;
         PK_File      : Ada.Streams.Stream_IO.File_Type;
         Elem         : Ada.Streams.Stream_Element;
         Key_Loaded   : Boolean := False;
      begin
         --  Check if key files exist
         if Ada.Directories.Exists (SK_Path) and then
            Ada.Directories.Exists (PK_Path)
         then
            --  Load secret key
            Open (SK_File, In_File, SK_Path);
            for I in SK'Range loop
               Ada.Streams.Stream_Element'Read (Stream (SK_File), Elem);
               SK (I) := Anubis_Types.Byte (Elem);
            end loop;
            Close (SK_File);

            --  Load public key
            Open (PK_File, In_File, PK_Path);
            for I in PK'Range loop
               Ada.Streams.Stream_Element'Read (Stream (PK_File), Elem);
               PK (I) := Anubis_Types.Byte (Elem);
            end loop;
            Close (PK_File);
         ...
         if not Key_Loaded then
            Print_Error ("Failed to load key: " & Key_Name_Str);
            Print_Info ("Run 'khepri keys new " & Key_Name_Str & "' first");
            Set_Exit_Status (Ada.Command_Line.Failure);
            return;
         end if;
      end;
      ...
      --  Format: [4 bytes: type=0x01 deploy] [4 bytes: code size] [code] [4 bytes: gas limit]
      Print_Progress (3, 5, "Building deployment transaction...");
      Tx_Data (0) := 16#01#;  -- Deploy transaction type
      Tx_Data (1) := 0;
      Tx_Data (2) := 0;
      Tx_Data (3) := 0;
      Tx_Data (4) := Aegis_VM_Types.Byte (Binary_Size mod 256);
      Tx_Data (5) := Aegis_VM_Types.Byte ((Binary_Size / 256) mod 256);
      Tx_Data (6) := Aegis_VM_Types.Byte ((Binary_Size / 65536) mod 256);
      Tx_Data (7) := Aegis_VM_Types.Byte ((Binary_Size / 16777216) mod 256);

      for I in 0 .. Binary_Size - 1 loop
         Tx_Data (8 + I) := Binary_Data (I);
      end loop;

      --  Append gas limit (4 bytes, little-endian)
      declare
         Gas : constant Natural := Natural (Options.Gas_Limit);
      begin
         Tx_Data (8 + Binary_Size + 0) := Aegis_VM_Types.Byte (Gas mod 256);
         Tx_Data (8 + Binary_Size + 1) := Aegis_VM_Types.Byte ((Gas / 256) mod 256);
         Tx_Data (8 + Binary_Size + 2) := Aegis_VM_Types.Byte ((Gas / 65536) mod 256);
         Tx_Data (8 + Binary_Size + 3) := Aegis_VM_Types.Byte ((Gas / 16777216) mod 256);
      end;
      Tx_Size := 8 + Binary_Size + 4;

      --  Sign the transaction
      Print_Progress (4, 5, "Signing transaction with ML-DSA-87...");
      declare
         --  Convert Tx_Data to Anubis_Types.Byte_Array for SHA3
         Tx_For_Hash : Anubis_Types.Byte_Array (0 .. Tx_Size - 1);
         Random_Seed : Anubis_MLDSA_Types.Seed := (others => 0);
         Sign_Success : Boolean;
      begin
         for I in 0 .. Tx_Size - 1 loop
            Tx_For_Hash (I) := Anubis_Types.Byte (Tx_Data (I));
         end loop;

         Anubis_SHA3.SHA3_256 (Tx_For_Hash, Msg_Hash);

         --  Generate a simple random seed from timestamp
         declare
            use Ada.Calendar;
            Now : constant Time := Clock;
            Secs : constant Day_Duration := Seconds (Now);
            Seed_Val : constant Unsigned_64 := Unsigned_64 (Float (Secs) * 1_000_000.0);
         begin
            Random_Seed (0) := Anubis_Types.Byte (Seed_Val mod 256);
            Random_Seed (1) := Anubis_Types.Byte ((Seed_Val / 256) mod 256);
            Random_Seed (2) := Anubis_Types.Byte ((Seed_Val / 65536) mod 256);
            Random_Seed (3) := Anubis_Types.Byte ((Seed_Val / 16777216) mod 256);
         end;

         Anubis_MLDSA.Sign (
            SK      => SK,
            Msg     => Anubis_Types.Byte_Array (Msg_Hash),
            Random  => Random_Seed,
            Sig     => Signature,
      ...
      for I in Signature'Range loop
         Tx_Data (Tx_Size + I) := Aegis_VM_Types.Byte (Signature (I));
      end loop;
      Tx_Size := Tx_Size + Signature'Length;

      if Options.Dry_Run then
         Print_Warning ("Dry run mode - transaction built but not sent");
         Print_Info ("  Transaction size: " & Tx_Size'Image & " bytes");
         Print_Info ("  Signature size: " & Natural'Image (Signature'Length) & " bytes");
         return;
      end if;

      --  Send transaction
      Print_Progress (5, 5, "Sending deployment transaction...");
      Send_Raw_Transaction (
         Endpoint   => Endpoint,
         Signed_Tx  => Tx_Data (0 .. Tx_Size - 1),
         Tx_Hash    => Tx_Hash,
         Error      => Error,
         Success    => Success
      );
      ...
      if not Success then
         Print_Error ("Failed to send transaction: " & RPC_Error_Code'Image (Error.Code));
         Set_Exit_Status (Ada.Command_Line.Failure);
         return;
      end if;

      Print_Info ("Transaction sent: 0x" & Hash_To_Hex (Tx_Hash));
      New_Line;
      ...
      if not Success then
         Print_Error ("Failed to get receipt: " & RPC_Error_Code'Image (Error.Code));
         Set_Exit_Status (Ada.Command_Line.Failure);
         return;
      end if;

      if not Found then
         Print_Warning ("Transaction pending - receipt not yet available");
         Print_Info ("Check status with: khepri tx " & Hash_To_Hex (Tx_Hash));
         return;
      end if;
      ...
      if Receipt.Status then
         ...
         Print_Info ("  Contract address: 0x" & Address_To_Hex (Receipt.Contract_Address));
         Print_Info ("  Block number: " & Aegis_VM_Types.Word64'Image (Receipt.Block_Number));
         Print_Info ("  Gas used: " & Aegis_VM_Types.Word64'Image (Receipt.Gas_Used));
      else
         Print_Error ("Deployment failed (transaction reverted)");
         Print_Info ("  Gas used: " & Aegis_VM_Types.Word64'Image (Receipt.Gas_Used));
         Set_Exit_Status (Ada.Command_Line.Failure);
      end if;
   end Handle_Deploy;
