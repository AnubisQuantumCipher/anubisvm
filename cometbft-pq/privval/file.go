...
func loadFilePV(keyFilePath, stateFilePath string, loadState bool) *FilePV {
	...
	// For ML-DSA-87, skip pubkey derivation - NIST FIPS 204 secret keys don't embed
	// the full public key, so we must use the stored pubkey from JSON
	if pvKey.PrivKey.Type() != "ml-dsa-87" {
		pvKey.PubKey = pvKey.PrivKey.PubKey()
	}
	// If pubkey wasn't loaded from JSON (nil), try to derive it
	if pvKey.PubKey == nil {
		pvKey.PubKey = pvKey.PrivKey.PubKey()
	}
