...
func (app *BaseApp) InitChain(req *abci.RequestInitChain) (*abci.ResponseInitChain, error) {
	...
	// ML-DSA-87: Skip strict validator comparison for post-quantum keys
	// The protobuf serialization may differ between CometBFT and SDK paths
	// but the actual key bytes are the same, verified during genesis setup
	if len(req.Validators) > 0 && len(res.Validators) > 0 {
		...
		// Compare only power and key bytes, not strict proto equality
		// This accommodates ML-DSA-87 which packs into Ed25519 field
		for i := range res.Validators {
			reqVal := req.Validators[i]
			resVal := res.Validators[i]
			app.logger.Info("InitChain validator comparison",
				"index", i,
				"req.Power", reqVal.Power,
				"res.Power", resVal.Power,
				"req.PubKey.Type", fmt.Sprintf("%T", reqVal.PubKey.Sum),
				"res.PubKey.Type", fmt.Sprintf("%T", resVal.PubKey.Sum),
			)
			if reqVal.Power != resVal.Power {
				return nil, fmt.Errorf("validator[%d] power mismatch: CometBFT=%d SDK=%d", i, reqVal.Power, resVal.Power)
			}
			// Key bytes comparison - both should be packed in Ed25519 field
			var reqKey, resKey []byte
			if ed := reqVal.PubKey.GetEd25519(); ed != nil {
				reqKey = ed
			}
			if ed := resVal.PubKey.GetEd25519(); ed != nil {
				resKey = ed
			}
			if len(reqKey) != len(resKey) {
				return nil, fmt.Errorf("genesisValidators[%d].PubKey len != req.Validators[%d].PubKey len (%d != %d)", i, i, len(resKey), len(reqKey))
			}
			for j := range reqKey {
				if reqKey[j] != resKey[j] {
					return nil, fmt.Errorf("genesisValidators[%d].PubKey bytes differ from req.Validators[%d].PubKey at position %d", i, i, j)
				}
			}
		}
	}
