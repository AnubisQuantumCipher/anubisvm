...
package body Khepri_RPC_Client is
   ...
   begin
      ...
      begin
         ...
         Nonce := Extract_JSON_Number (JSON, "result");
         Success := True;
      end;
   ...
   begin
      ...
      begin
         ...
         Gas_Price := Extract_JSON_Number (JSON, "result");
         Success := True;
      end;
   end Get_Gas_Price;
   ...
   procedure Estimate_Gas (
      Endpoint     : in     RPC_Endpoint;
      From_Address : in     Khepri_Types.Address;
      To_Address   : in     Khepri_Types.Address;
      Data         : in     Byte_Array;
      Value        : in     Word64;
      Gas_Estimate : out    Word64;
      Error        : out    RPC_Error;
      Success      : out    Boolean
   ) is
      From_Hex : constant String := Address_To_Hex (From_Address);
      To_Hex   : constant String := Address_To_Hex (To_Address);
      Data_Hex : constant String := To_Hex_String (Data);
      Value_Hex : constant String := "0x" & Byte_To_Hex (Byte (Value mod 256));
      JSON_Request : constant String := Build_JSON_RPC_Request (
         "anubis_estimateGas",
         "{""from"":""" & From_Hex & """," &
         """to"":""" & To_Hex & """," &
   ...
   begin
      ...
      begin
         ...
         Gas_Estimate := Extract_JSON_Number (JSON, "result");
         Success := True;
      end;
   end Estimate_Gas;
